...
def files_set() -> Generator:
    with patch('src.utils.transactions_from_json') as mock:
...
def test_files_set(files_set: None) -> None:
    ...
    transactions = transactions_from_json("data/test_operations.json")
    assert transactions == data


def test_files_set_empty(files_set: None) -> None:
    open("data/empty_operations.json", "a").close()
    transactions = transactions_from_json("data/empty_operations.json")
    assert transactions == []
...
def test_files_not_found(files_set: None) -> None:
    if os.path.exists("data/not_list_operations.json"):
        ...
        transactions = transactions_from_json("data/not_list_operations.json")
        assert transactions == []
